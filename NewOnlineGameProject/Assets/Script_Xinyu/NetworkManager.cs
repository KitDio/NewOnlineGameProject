...
public class NetworkManager : MonoBehaviourPunCallbacks
{
    ...
    public void OnStartGameButtonClicked()
    {
        ...
        {
            // 1. 获取当前房间的难度
            string currentDifficulty = "Normal"; // 给个默认兜底值
            if (PhotonNetwork.CurrentRoom.CustomProperties.ContainsKey("Difficulty"))
            {
                currentDifficulty = (string)PhotonNetwork.CurrentRoom.CustomProperties["Difficulty"];
            }

            // 2. 根据不同的难度，让 Photon 加载不同的场景！
            if (currentDifficulty == "Difficult")
            {
                Debug.Log("正在加载高难度地图...");
                PhotonNetwork.LoadLevel("Difficult_Scene");
            }
            else
            {
                Debug.Log("正在加载普通难度地图...");
                PhotonNetwork.LoadLevel("Normal_Scene");
            }
        }
    }
